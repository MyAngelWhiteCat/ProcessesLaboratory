...
### Бэклог
- Рефакторинг rwx_analyzer.cpp соблюдая принципы DRY.
- Тестирование ядра сбора информации о процессах
- Тестирование RWX сканнера

### Исправления - Архитектура 
- [LOW] ProcessScanner делает как сканирования, так и запрашивает анализ. Требуется разработать отдельный класс лабаратории, который будет проводить все необходимые анализы, а ProcessScanner должен отвечать только за создание снапшотов и их эффективное хранение. Немедленное исправление не требуется, задачка на 10 минут и не будет усложняться, так как добавление новых анализаторов не ведет за собой усложнение готового ядра сканирования. Соответсвенно простым решением будет - Переименовать ProcessScanner на ProcessLabaratory сохранив всю логику анализа. Создать отдельный файл и класс ProcessScanner, в который вынести всю текущую логику сканирования и хранения снапшотов.

### Произоводительность - thread_pool
- реализовать 2 пула потоков - для Application и для системы в целях повышения производительности путем параллельного сканирования и анализа. 

### Фича - UI
- Сделать простейший демонстративный UI с использоваием WinMain WndProc 
